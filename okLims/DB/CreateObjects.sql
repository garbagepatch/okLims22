...
IF NOT EXISTS (SELECT 1 FROM [INFORMATION_SCHEMA].[TABLES] WHERE [TABLE_NAME] = N'Request')
BEGIN
	CREATE TABLE dbo.Request (
	  ...
	  [Sum] money NOT NULL CONSTRAINT DF_Request_Sum DEFAULT (0),
	  [State] nvarchar(1024) NOT NULL DEFAULT ('VacationRequestCreated'),
	  StateName nvarchar(1024) ,
	  CONSTRAINT PK_Request PRIMARY KEY (Id),
	  CONSTRAINT FK_Request_Employee FOREIGN KEY (ManagerId) REFERENCES dbo.Employee (Id),
	  CONSTRAINT FK_Request_Employee1 FOREIGN KEY (AuthorId) REFERENCES dbo.Employee (Id) ON DELETE CASCADE ON UPDATE CASCADE
	)

	PRINT 'Request CREATE TABLE'
END

IF NOT EXISTS (SELECT 1 FROM [INFORMATION_SCHEMA].[TABLES] WHERE [TABLE_NAME] = N'RequestTransitionHistory')
BEGIN
	CREATE TABLE dbo.RequestTransitionHistory (
	  Id uniqueidentifier NOT NULL,
	  ...
	  CONSTRAINT PK_RequestTransitionHistory PRIMARY KEY (Id),
	  CONSTRAINT FK_RequestTransitionHistory_Request FOREIGN KEY (RequestId) REFERENCES dbo.Request (Id) ON DELETE CASCADE ON UPDATE CASCADE,
	  CONSTRAINT FK_RequestTransitionHistory_Employee FOREIGN KEY (EmployeeId) REFERENCES dbo.Employee (Id)
	)

	PRINT 'RequestTransitionHistory CREATE TABLE'
END
